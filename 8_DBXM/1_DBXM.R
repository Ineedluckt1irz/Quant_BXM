# ==============================================================================
# DBXM (Dual-Condition: VIX + MA Filter) - Portfolio Version
# 執行規則：只有在「VIX 高波動」且「MA 非強勢多頭」時才賣個股 Call
# ==============================================================================
...
path_output    <- file.path(getwd(), "Output_DBXM") # 建議另存路徑
...
for (idx in seq_along(all_tickers)) {
  ...
  tryCatch({
    ...
    for (i in 1:nrow(schedule)) {
      ...
      # 2. 取得 MA 因子 (MABXM)
      ma_info <- ma_signals %>% filter(Ticker == test_ticker, Trade_Date == curr_date)
      ...
      status_msg <- "DBXM(VIX+MA): B&H (Cond not met)"
      
      # 3. 雙因子決策核心：VIX 高波動 (1) AND MA 非強勢多頭 (0) [cite: 50, 185]
      ...
      results_list[[i]] <- data.frame(
        ...
        VIX_Signal=vix_sig, MA_No_Write=ma_no_write,
    ...
    if(!is.null(df_final) && nrow(df_final) > 0) {
      ...
      col_names <- c("DBXM (VIX+MA)", paste0("Buy & Hold (", test_ticker, ")"), "Market (DJITR)")
      ...
      saveWorkbook(wb, paste0(path_output, "/DBXM_VIX_MA_", test_ticker, ".xlsx"), overwrite = TRUE)
...
message(paste("\nDBXM (VIX+MA) 量產完成。成功處理總數:", success_count))