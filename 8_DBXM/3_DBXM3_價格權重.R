# ==============================================================================
# 模組：DBXM (VIX+MA) 投資組合整合 (價格權重聚合標準版)
# 功能：將各個個股的雙因子回測結果聚合為投組績效，完全對齊 DJIA 價格加權規格
# ==============================================================================
...
output_filename     <- "DBXM_VIX_MA_Portfolio_Final.xlsx"
...
# ==============================================================================
# 4. Step 1: 讀取 DBXM (VIX+MA) 個股回測結果 (使用 list 收集以解決 rbind 報錯)
# ==============================================================================
message("🔍 Step 1: 讀取 DBXM (VIX+MA) 個股回測檔...")
file_list <- list.files(path_input_folder, pattern = "DBXM_VIX_MA_.*\\.xlsx", full.names = TRUE)
...
for (f in file_list) {
  ...
  ticker <- str_match(basename(f), "DBXM_VIX_MA_(.*)\\.xlsx")[2]
  ...
  df_tmp <- tryCatch({
    ...
    tmp %>% mutate(
      ...
      Implied_Cost_DBXM = Term_Value_DBXM / (1 + dbxm_return),
      Term_Value_BnH = close_t1 + dividend,
      Implied_Cost_BnH = Term_Value_BnH / (1 + bnh_return)
    ) %>% filter(!is.na(dbxm_return))
  }, error = function(e) { message(paste("⚠️ 讀取失敗:", ticker)); NULL })
...
# ==============================================================================
message("🔄 Step 3: 計算 DBXM (VIX+MA) 價格權重聚合績效...")
...
# ==============================================================================
# 7. Step 4: 輸出指標與繪圖 (含暫存圖檔清理)
# ==============================================================================
message("📊 Step 4: 產出報表與績效矩陣...")

ts_cols  <- c("DBXM_Return", "BnH_Return", "DIA_Return", "DJITR_Return")
ts_names <- c("DBXM(VIX+MA)", "BnH", "DIA(ETF)", "DJITR")
...
perf_matrix <- rbind(tab_ann, "Max Drawdown"=mdd, "Sortino Ratio"=sortino, "Calmar Ratio"=calmar, 
...
temp_plot <- file.path(path_input_folder, "DBXM_VIX_MA_Portfolio_Chart.png")
png(temp_plot, width = 1400, height = 800, res = 120)
charts.PerformanceSummary(ts_ret_plot, wealth.index=TRUE, main="DBXM (VIX+MA) Performance (Benchmark: DJITR)", 
                          legend.loc="topleft", colorset=c("purple", "#778899", "grey", "black"), lwd=2)
dev.off()
...
message(paste("🎉 DBXM (VIX+MA) 投組整合報表已輸出:", output_filename))