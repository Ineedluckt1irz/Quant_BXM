...
load_final_ret <- function(p, m_val) {
  ...
  date_col <- names(df)[1] 
  target_col <- names(df)[grepl("IBXM", names(df))][1]
  
  df_sub <- df %>%
    select(all_of(c(date_col, target_col))) %>%
    mutate(across(-1, ~as.numeric(as.character(.)))) %>% 
    rename(Date = !!date_col, !!paste0("IBXM (m=", m_val, ")") := !!target_col) %>%
    mutate(Date = as.Date(Date))
  return(df_sub)
}

# 3. 讀取與合併資料
df_m100 <- load_final_ret(paths_ibxm$m100, "1.00")
df_m102 <- load_final_ret(paths_ibxm$m102, "1.02")
df_m104 <- load_final_ret(paths_ibxm$m104, "1.04")

# 讀取 DIA 資料 (基準)
df_dia_raw <- read_excel(paths_ibxm$m100, sheet = "Monthly_Returns")
...
# 5. 顏色與線條設定
# DIA 為灰色，其餘為藍色系由淺到深
my_colors <- c("#778899", "#ADD8E6", "#6495ED", "#00008B")
# 修改處：將第一個元素（DIA）改為 6 (虛點線)，其餘維持 1 (實線)
my_lty    <- c(6, 1, 1, 1) 
# 【新增】自訂每條線的粗細 (順序對應：DIA, m=1.00, m=1.02, m=1.04)
# 你可以在這裡更改數值，例如把 DIA 調粗改成 2.5
my_lwd    <- c(1.8, 1.8, 2.1, 2.4) 
...
export_file <- file.path(export_dir, "IBXM_Moneyness_Comparison.png")
...
# 主標題與日期標籤
mtext("Cumulative Returns: DIA vs IBXM at different Moneyness",
...
dev.off()
message(paste("✅ 圖表已對齊標準格式，匯出至：", export_file))