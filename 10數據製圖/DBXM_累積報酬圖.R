# ==========================================
# DBXM vs DIA 策略績效比較圖 (紫色調梯度版 - 自訂粗細)
# ==========================================
...
# 2. 讀取資料函數 (目標列改為搜尋 "DBXM")
load_final_ret <- function(p, m_val) {
  df <- read_excel(p, sheet = "Monthly_Returns")
  date_col <- names(df)[1] 
  target_col <- names(df)[grepl("DBXM", names(df))][1]
  
  df_sub <- df %>%
    select(all_of(c(date_col, target_col))) %>%
    mutate(across(-1, ~as.numeric(as.character(.)))) %>% 
    rename(Date = !!date_col, !!paste0("DBXM (m=", m_val, ")") := !!target_col) %>%
    mutate(Date = as.Date(Date))
  return(df_sub)
}

# 3. 讀取與合併資料
df_m100 <- load_final_ret(paths_dbxm$m100, "1.00")
df_m102 <- load_final_ret(paths_dbxm$m102, "1.02")
df_m104 <- load_final_ret(paths_dbxm$m104, "1.04")

# 讀取 DIA 資料 (作為基準 ETF)
df_dia_raw <- read_excel(paths_dbxm$m100, sheet = "Monthly_Returns")
...
# 5. 顏色與線條設定
# 調整重點：DBXM 顏色設定為紫色調，越價外越深
my_colors <- c("#778899", "#9575CD", "#5E35B1", "#311B92")# DIA (第一個元素) 維持 6 (點虛線)，其餘策略為實線 1
my_lty    <- c(6, 1, 1, 1) 

# 【新增】自訂每條線的粗細 (順序對應：DIA, m=1.00, m=1.02, m=1.04)
# 你可以在這裡更改數值，目前預設皆為 1.8
my_lwd    <- c(1.8, 1.8, 2.0, 2.6)
...
export_file <- file.path(export_dir, "DBXM_Moneyness_Comparison.png")
...
# 主標題與日期標籤
mtext("Cumulative Returns: DIA vs DBXM at different Moneyness",
...
dev.off()
message(paste("✅ 已完成紫色系 DBXM vs DIA 績效圖，匯出至：", export_file))