# ==============================================================================
# MABXM (Moving Average Based Buy-Write Model) - Trend Follower Version
# ==============================================================================
...
# 讀取 MA 訊號檔並校正日期格式 [cite: 206]
message("正在讀取 MA 訊號檔...")
ma_signal_data <- read.xlsx(path_ma_csv) %>%
  mutate(
...
for (idx in seq_along(all_tickers)) {
  ...
  tryCatch({
    ...
    for (i in 1:nrow(schedule)) {
      ...
      # 【核心改動】：獲取 MA 訊號判定 [cite: 162, 169, 172]
      current_ma_info <- ma_signal_data %>% 
        filter(Ticker == test_ticker, Trade_Date == curr_date)
      
      # 若 20MA > 180MA, Action_No_Write = 1 (不賣買權) [cite: 169]
      no_write_signal <- if(nrow(current_ma_info) > 0) as.numeric(current_ma_info$Action_No_Write[1]) else 0
      ...
      strike_selected <- NA; premium_received <- 0; status_msg <- "MA Bullish: Buy & Hold Only"
      
      # MABXM 邏輯：Action_No_Write 為 0 時才執行 Buy-Write 
        ...
        if(!is.null(opt_raw)) {
          ...
          if(nrow(target_opts) > 0) {
            ...
            if(nrow(candidates) > 0) {
              ...
              status_msg <- "MA Non-Bullish: Sell Call (Buy-Write)"
            }
          }
        }
      }
      
      # [D] 結算 [cite: 111, 112, 113]
      ...
      results_list[[i]] <- data.frame(
        Trading_Date=curr_date, Expiry_Date=next_date, Ticker=test_ticker, No_Write_Signal=no_write_signal,
    ...
    if(!is.null(df_final) && nrow(df_final) > 0) {
      ...
      col_names <- c(paste0("MABXM (M=", moneyness, ")"), paste0("Buy & Hold (", test_ticker, ")"), "Market (DJITR)")
      ...
      charts.PerformanceSummary(ts_ret_plot, main = paste("Strategy Performance (MABXM):", test_ticker), wealth.index = TRUE, colorset = c("darkgreen", "blue", "gray"), lwd = 2)
      dev.off()
      ...
      # 檔案命名為 MABXM
      saveWorkbook(wb, paste0(path_output, "/MABXM_", test_ticker, ".xlsx"), overwrite = TRUE)
...
message(paste("\nMABXM 量產回測完成。成功處理:", success_count))