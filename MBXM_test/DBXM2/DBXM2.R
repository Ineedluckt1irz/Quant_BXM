...
message("正在載入 DBXM 雙因子優化版資料 (聯集邏輯)...")
...
# 載入排名資料 (MBXM 因子：直接讀取已判定好的 Is_Strong_Stock)
...
trade_dates <- raw_dates[!is.na(raw_dates)]

schedule_global <- data.frame(Date_Roll = trade_dates, Date_Expiry = lead(trade_dates)) %>% na.omit()
...
for (idx in seq_along(all_tickers)) {
  ...
  tryCatch({
    ...
    for (i in 1:nrow(schedule)) {
      ...
      # 2. 取得強勢股判定 (MBXM 因子：直接使用檔案判斷結果)
      target_rank_info <- rank_data_all %>% filter(Trade_Date == curr_date, Ticker == test_ticker)
      is_strong <- if(nrow(target_rank_info) > 0) as.numeric(target_rank_info$Is_Strong_Stock[1]) else 0
      ...
      status_msg <- "DBXM: Buy & Hold (Signal Not Met)"
      
      # 3. DBXM 決策核心：聯集邏輯 (市場恐慌 OR 個股非強勢)
      # 目的：只要符合其一就賣 Call，增加權利金緩衝
        ...
        if(!is.null(opt_raw)) {
          ...
          if(nrow(target_opts) > 0) {
            ...
            if(nrow(candidates) > 0) {
              ...
              status_msg <- "DBXM: Sell Call (Union Signal)"
            }
          }
        }
      }
    ...
    if(!is.null(df_final) && nrow(df_final) > 0) {
      ...
      charts.PerformanceSummary(ts_ret_plot, main = paste("Strategy Performance (DBXM-Union):", test_ticker), wealth.index = TRUE, colorset = c("purple", "blue", "gray"), lwd = 2)
      dev.off()
...
message(paste("\nDBXM 雙因子優化版量產完成。成功處理總數:", success_count))