# ==============================================================================
# MBXM (Momentum-Based Buy-Write Model) - Sharpe Momentum Integration Version
# 核心優化：直接讀取排名檔 Is_Strong_Stock 判定，對齊夏普動能篩選結果
# ==============================================================================
...
for (idx in seq_along(all_tickers)) {
  ...
  tryCatch({
    ...
    for (i in 1:nrow(schedule)) {
      ...
      strike_selected <- NA; premium_received <- 0; status_msg <- "Sharpe Momentum Strong: Buy & Hold"
      
      # MBXM 邏輯：只有強勢股 (Is_Strong_Stock == 1) 才不賣權
      if (is_strong == 0) {
        ...
        if(!is.null(opt_raw)) {
          ...
          if(nrow(target_opts) > 0) {
            ...
            if(nrow(candidates) > 0) {
              ...
              status_msg <- "Sharpe Momentum Weak: Sell Call"
            }
          }
        }
      }
