# ==============================================================================
# MBXM (Momentum-Based Buy-Write Model) - Special Cases: RTX, DD, MDLZ
# 交易規則：只有在「弱勢股 (MBXM Non-Top)」時才賣個股 Call；強勢股則 Buy & Hold
# ==============================================================================
...
# 4. 讀取基礎資料 (SQL, Rank) =====
message(paste("🔍 正在讀取", target_ticker, "的 MBXM 基準資料..."))
...
tryCatch({
  ...
  for (i in 1:nrow(schedule)) {
    ...
    # MBXM 信號匹配 (僅看動能)
    mom_info  <- rank_data %>% filter(Ticker == target_ticker) %>% mutate(d=abs(as.numeric(Trade_Date - curr_date))) %>% filter(d<=3) %>% arrange(d) %>% slice(1)
    is_strong <- if(nrow(mom_info) > 0) as.numeric(mom_info$Is_Strong_Stock[1]) else 0
    ...
    strike_selected <- NA; premium_received <- 0; status_msg <- "MBXM: Momentum Strong (Buy & Hold)"
    
    # 【MBXM 核心決策】：若非強勢股 (is_strong == 0)，則賣 Call
    if (is_strong == 0) {
      ...
      if(!is.null(opt_raw)) {
        ...
        if(nrow(target_opts) > 0) {
          ...
          if(nrow(candidates) > 0) {
            ...
            status_msg <- "MBXM: Sell Call (Weak Stock)"
          }
        }
      }
    }
    ...
    results_list[[i]] <- data.frame(
      ...
      Is_Strong=is_strong, Moneyness=moneyness, Split=cfadj_T,
  ...
  if(!is.null(df_final) && nrow(df_final) > 0) {
    ...
    colnames(ts_data) <- c("MBXM (Momentum)", paste0("Buy & Hold (", target_ticker, ")"), "Market (DJITR)")
    ...
    temp_plot <- paste0(path_output, "/chart_MBXM_", target_ticker, ".png")
    png(temp_plot, width = 1200, height = 800, res = 120)
    charts.PerformanceSummary(ts_ret_plot, main = paste("MBXM Performance:", target_ticker), wealth.index = TRUE, colorset = c("darkgreen", "blue", "gray"), lwd = 2)
    dev.off()
    ...
    saveWorkbook(wb, paste0(path_output, "/MBXM_", target_ticker, ".xlsx"), overwrite = TRUE)
    if(file.exists(temp_plot)) file.remove(temp_plot)
    message(paste("🎉", target_ticker, "MBXM 特殊案件量產完成。"))
  }
