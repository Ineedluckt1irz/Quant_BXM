# ==============================================================================
# DBXM (Dual-Condition Buy-Write Model) - Special Cases: RTX, DD, MDLZ
# 更新版：個股因子同步改為「夏普動能 (Sharpe Momentum)」判定標準
# 只有在「恐慌期 (VIX High)」+「非夏普強勢股 (Sharpe-Weak)」時才賣個股 Call
# ==============================================================================
...
message(paste("🔍 正在讀取", target_ticker, "的 DBXM 基準資料 (同步夏普動能)..."))
...
# 【關鍵修正】：讀取排名資料並建立夏普排名 (MBXM 因子同步)
rank_data_all <- read.xlsx(path_rank_csv) %>%
...
tryCatch({
  ...
  for (i in 1:nrow(schedule)) {
    ...
    # 2. 取得夏普動能排名 (同步新規則)
    daily_ranks <- rank_data_all %>% filter(Trade_Date == curr_date) %>%
      arrange(desc(Sharpe_Mom)) %>% 
      mutate(Sharpe_Rank = row_number())
    
    target_rank_info <- daily_ranks %>% filter(Ticker == target_ticker)
    is_strong <- if(nrow(target_rank_info) > 0 && target_rank_info$Sharpe_Rank[1] <= top_n_threshold) 1 else 0
    ...
    strike_selected <- NA; premium_received <- 0; status_msg <- "DBXM: Buy & Hold (Signal Not Met)"
    
    # 【DBXM 核心決策】：VIX High & Sharpe-Momentum Weak
      ...
      if(!is.null(opt_raw)) {
        ...
        if(nrow(target_opts) > 0) {
          ...
          if(nrow(candidates) > 0) {
            ...
            status_msg <- "DBXM: Sell Call (Panic + Sharpe-Weak)"
          }
        }
      }
    }
    ...
    results_list[[i]] <- data.frame(
      ...
      VIX_Signal=vix_sig, Is_Strong=is_strong, Moneyness=moneyness, Split=cfadj_T,
  ...
  if(!is.null(df_final) && nrow(df_final) > 0) {
    ...
    colnames(ts_data) <- c("DBXM (Dual-Cond)", paste0("Buy & Hold (", target_ticker, ")"), "Market (DJITR)")
    ...
    charts.PerformanceSummary(ts_ret_plot, main = paste("DBXM Performance:", target_ticker), wealth.index = TRUE, colorset = c("purple", "blue", "gray"), lwd = 2)
    dev.off()
    ...
    saveWorkbook(wb, paste0(path_output, "/DBXM_", target_ticker, ".xlsx"), overwrite = TRUE)
    if(file.exists(temp_plot)) file.remove(temp_plot)
    message(paste("🎉", target_ticker, "DBXM 特殊案件量產完成。"))
  }
